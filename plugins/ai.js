    ...
    try {
      ...
      if (contentType && contentType.startsWith('image')) {
        ...
        await m.bot.sendFromUrl(
          m.from,
          photoUrl,
          "*Queen_CHARITY generated this photo*:",
          m,
          {},
          "image"
        );
        ...
        await m.bot.sendFromUrl(
          m.from,
          photoUrl,
          "*Queen_CHARITYgenerated this photo*:",
          m,
          {},
          "image"
        );
    ...
    async (_0x2acf30) => {
      try {
        if (
          ...
          if (
            _0x2acf30.reply_text.includes("QUEEN_CHARITY• ᴀɴɴᴏɴʏᴍᴏᴜs ᴍsɢ") &&
            _0x2dfb59[0].includes("QUEEN_CHARITY • ᴀɴɴᴏɴʏᴍᴏᴜs ᴍsɢ") &&
...
smd(
  {
    pattern: "charity",
    desc: "Get a response from CHARITY-AI based on user query.",
    category: "ai",
    filename: __filename,
  },
  async (m) => {
    try {
      ...
      const query = m.text.split(' ').slice(1).join(' ');
      if (!query) {
        return await m.send("Please provide a query, e.g., `,CHARITY What is the weather today?`.");
      }
